...
public class Listeners extends BaseClass implements ITestListener {
	...
	ExtentReports extent = ExtendReportUtility.createExtentReports();  //ExtentReports object extent initializes the reporting framework using a utility class (ExtentReportUtility).
	...
	public void onTestFailure (ITestResult result) {
		...
		WebDriver driver = null; String testMethodName = result.getMethod().getMethodName(); try {

		driver = (WebDriver) result.getTestClass().getRealClass().getDeclaredField("driver") .get(result.getInstance()); } catch (IllegalArgumentException e) {
